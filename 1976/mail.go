...
func newCityMap(size int, roads [][]int) *CityMap {
	c := CityMap{Intersections: make([]*Intersection, size), LastIndex: size - 1}
	for i := 0; i < size; i++ {
		c.Intersections[i] = &Intersection{number: i, timeToReach: -1}
	}

	c.Intersections[0].timeToReach = 0
	c.Intersections[0].shortWays = 1
	for _, road := range roads {
		c.Intersections[road[0]].roads = append(c.Intersections[road[0]].roads, Road{destination: c.Intersections[road[1]], time: road[2], d: road[1]})
		c.Intersections[road[1]].roads = append(c.Intersections[road[1]].roads, Road{destination: c.Intersections[road[0]], time: road[2], d: road[0]})
	}
	return &c
}

func (c *CityMap) FindWays() int {
	if len(c.Intersections) < 1 {
		return 0
	}
	for _, intersection := range c.Intersections {
		if intersection.timeToReach == -1 {
			continue
		}
		for _, road := range intersection.roads {
			t := intersection.timeToReach + road.time
			if road.destination.timeToReach > t || road.destination.timeToReach == -1 {
				// if this point is reached for the first time or new way is faster,
				// then we have to reassign the shortest time and number of short ways
				road.destination.timeToReach = t
				road.destination.shortWays = intersection.shortWays
				road.destination.shortCameFrom = []int{intersection.number}
			} else if road.destination.timeToReach < t {
				// if shorter way is exists then skip current way
				continue
			} else {
				// if the same short way exists then we have to sum up the ways count
				road.destination.shortWays = (road.destination.shortWays + intersection.shortWays) % 1000000007
				road.destination.shortCameFrom = append(road.destination.shortCameFrom, intersection.number)
			}
		}
	}
	fmt.Println(c)
	return c.Intersections[c.LastIndex].shortWays % 1000000007
}

func (c *CityMap) FindViaQueue() int {
	queue := [][]int{{0, 1}}

	for len(queue) > 0 {
		// sort queue to process closest intersections first
		sort.Slice(queue, func(i, j int) bool {
			return queue[i][1] < queue[j][1]
		})
		intersection := c.Intersections[queue[0][0]]
		queue = queue[1:]
		if intersection.processed {
			continue
		}
		intersection.processed = true
		for _, road := range intersection.roads {
			t := intersection.timeToReach + road.time
			if road.destination.timeToReach > t || road.destination.timeToReach == -1 {
				// if this point is reached for the first time or new way is faster,
				...
				road.destination.shortWays = intersection.shortWays
				road.destination.shortCameFrom = []int{intersection.number}
				queue = append(queue, []int{road.destination.number, t})
			} else if road.destination.timeToReach < t {
				// if shorter way is exists then skip current way
				continue
			} else {
